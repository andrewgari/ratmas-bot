...
def has_participant_role():
    ...
    async def predicate(interaction: discord.Interaction) -> bool:
        ...
        await interaction.response.send_message(
            "❌ You need the participant role to use this command.",
            ephemeral=True
        )
        return False
...
async def setup_participant_commands(bot, db: "RatmasDB"):
    ...
    @bot.tree.command(name="list-packages", description="See how many packages are being sent to you")
    ...
    async def list_packages(interaction: discord.Interaction):
        ...
        if total_packages == 0:
            message = "🎁 **The Rats are still hunting for your gifts!** 📦\n\nNo packages have been reported yet."
        else:
            message = f"🎁 **The rats have sent {total_packages} package{'s' if total_packages != 1 else ''} to you!** 📦"
        
        await interaction.response.send_message(message, ephemeral=True)
        logger.info(f"User {interaction.user.name} checked status: {total_packages} packages")
    
    @bot.tree.command(name="update-packages", description="Update how many packages you're sending to people")
    @has_participant_role()
    async def update_packages(interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.response.send_message(
                "✅ Check your DMs to update package counts!",
                ephemeral=True
            )
