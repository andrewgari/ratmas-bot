...
async def setup_participant_commands(bot, db: "RatmasDB"):
    ...
    @bot.tree.command(
        name="list-packages", description="See how many packages are being sent to you"
    )
    ...
    @bot.tree.command(
        name="update-packages", description="Update how many packages you're sending to people"
    )
    @has_participant_role()
    async def update_packages(interaction: discord.Interaction):
        ...
        if not db.is_season_active():
            await interaction.response.send_message("❌ No active Ratmas season.", ephemeral=True)
            return
