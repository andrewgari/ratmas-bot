...
async def setup_admin_commands(bot, db: "RatmasDB"):
    ...
    async def start_ratmas(interaction: discord.Interaction):
        ...
        if db.is_season_active():
            await interaction.response.send_message(
                "❌ Ratmas is already in progress. Please run `/end-ratmas` first.",
                ephemeral=True
            )
            return
    ...
    @bot.tree.command(name="end-ratmas", description="[ADMIN] End the gift exchange and archive all data")
    @app_commands.default_permissions(administrator=True)
    ...
    async def end_ratmas(interaction: discord.Interaction, permanent: bool = False):
        ...
        if not db.is_season_active():
            await interaction.response.send_message(
                "❌ No active Ratmas season to end.",
                ephemeral=True
            )
            return
    ...
    @bot.tree.command(name="custom-assignments", description="[ADMIN] Send DMs so participants can choose who they're sending gifts to")
    @app_commands.default_permissions(administrator=True)
    async def custom_assignments(interaction: discord.Interaction):
        """Send DMs to participants to select their rats."""
        if not db.is_season_active():
            await interaction.response.send_message(
                "❌ No active Ratmas season. Please run `/start-ratmas` first.",
                ephemeral=True
            )
            return
        ...
        if len(participants) < 2:
            await interaction.followup.send(
                f"❌ Need at least 2 participants. Found {len(participants)}.",
                ephemeral=True
            )
            return
        ...
        logger.info(f"Custom assignments initiated by {interaction.user.name}: {success_count} sent")
    
    @bot.tree.command(name="package-update-query", description="[ADMIN] Send DMs asking everyone to update how many packages they're sending")
    @app_commands.default_permissions(administrator=True)
    async def package_update_query(interaction: discord.Interaction):
        ...
        if not users:
            await interaction.followup.send(
                "❌ No participants found. Run `/custom-assignments` first.",
                ephemeral=True
            )
            return
        ...
        for user_data in users:
            try:
                ...
                logger.error(f"Failed to send package update DM to {user_data['display_name']}: {e}")
        ...
        await interaction.followup.send(message, ephemeral=True)
        logger.info(f"Package update query initiated by {interaction.user.name}: {success_count} sent")
