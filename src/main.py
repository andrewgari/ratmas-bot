...
class RatmasBot(discord.Client):
    ...
    async def on_member_remove(self, member: discord.Member):
        ...
        try:
            ...
            sending_to = [
                a for a in assignments if a["sender_id"] == member.id and a.get("is_official")
            ]
            receiving_from = [
                a for a in assignments if a["receiver_id"] == member.id and a.get("is_official")
            ]
            ...
            logger.warning(
                f"User {member.display_name} ({member.id}) left server - manager notified"
            )
