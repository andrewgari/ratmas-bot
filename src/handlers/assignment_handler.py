    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.db.has_official_sender(selected_id):
            await interaction.response.send_message(
                "❌ That person already has a sender. Please pick someone else.",
                ephemeral=True
            )
            logger.warning(f"User {self.user_id} tried to select {selected_id} who already has a sender")
            return
...
async def check_assignments_complete(bot, db: "RatmasDB"):
    ...
    if len(official_assignments) == len(all_users):
        ...
        for user in all_users:
            ...
            has_assignment = any(
                a["sender_id"] == user_id and a.get("is_official")
                for a in official_assignments
            )
