...
class PackageCountModal(ui.Modal, title="Enter Package Count"):
    ...
    def __init__(self, sender_id: int, target_user_id: int, target_display_name: str, is_official: bool, db: "RatmasDB"):
        super().__init__()
...
class RecipientSelect(ui.Select):
    ...
    def __init__(self, sender_id: int, all_users: list, official_receiver_id: int, db: "RatmasDB"):
        self.sender_id = sender_id
        self.db = db
        ...
        for user in all_users:
            if user["user_id"] != sender_id:
                is_official = (user["user_id"] == official_receiver_id)
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        is_official = (target_user_id == self.official_receiver_id)
...
class PackageUpdateView(ui.View):
    ...
    def __init__(self, sender_id: int, all_users: list, official_receiver_id: int, bot, db: "RatmasDB"):
    ...
    async def done_callback(self, interaction: discord.Interaction):
        ...
        try:
            await interaction.response.send_message(
                "✅ **All done!** Your package counts have been saved.",
                ephemeral=True
            )
        except Exception as e:
